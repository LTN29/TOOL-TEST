...
CREATE TABLE IF NOT EXISTS comment_jobs (
  ...
  status ENUM('PENDING','READY_FOR_WORKER','RUNNING','SUCCESS','FAILED','UNKNOWN','RETRY_DUE','SKIPPED','PAUSED','CANCELLED') NOT NULL DEFAULT 'PENDING',
  dry_run BOOLEAN NOT NULL DEFAULT TRUE,
  ...
  CONSTRAINT fk_jobs_group FOREIGN KEY (group_id) REFERENCES comment_groups(id) ON DELETE RESTRICT,
